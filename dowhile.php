<!DOCTYPE html>
 ...
 <?php
  $WasError = 0; # индикатор ошибки - если не 0, то была ошибка
    if(isset($_REQUEST['doSubmit'])) do {  // Если нажали кнопку Submit (с именем $doSubmit)...
    if($REQUEST['reloads'] != 1+1+7) { $WasError = 1; break; } // Проверка входных данных
    if($REQUEST['loader'] != "source" ) { $WasError = 1; break; }
    ...
    # В этой точке данные точно в порядке. Обрабатываем их.
      ...
      echo "Вы внимательный человек, поздравляем!<br />"; // Можно записать данные в файл.
        ...
        exit();
          }while(0); # Произошла ли ошибка?
            if($WasError) {
              echo "Вы ответили неверно, попробуйте еще раз.";  
                }
          ?>
      <!-- Выводим форму, через которую пользователь будет запускать этот сценарий, и, возможно, отображаем сообщение об ошибке в случае,
если $WasError != 0. -->

         <form action="<?=$_SERVER['REQUEST_URI']?>" method="POST">
          Число перезагрузок: <input type="text" name="reloads"><br />
          3агрузочная программа: <input type="text" name="loader"><br />
